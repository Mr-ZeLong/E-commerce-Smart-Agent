...
def login_user(username: str, password: str) -> tuple[bool, str, ChatClient | None, str]:
    """
    用户登录
...
def create_chat_interface():
    ...
    with gr.Blocks(title="Smart Agent v4.0", theme=theme, css=custom_css) as demo:
        ...
        #  登录界面
        # ====================
        with gr.Group(visible=True) as login_panel, gr.Column(elem_classes="login-wrapper"):
            gr.HTML("""
        ...
        with gr.Group(visible=False) as chat_panel:
            ...
            # 顶部导航栏
            with gr.Group(elem_classes="nav-header"), gr.Row(equal_height=True):
        ...
        def handle_login(username, password):
            ...
            if success:
                # 提取姓名用于 Header 显示
                name = client.username  # ty:ignore[unresolved-attribute]
                header_html = f'''
