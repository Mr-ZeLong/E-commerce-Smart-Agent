...
def _process_refund_payment_body(
    self, refund_id: int, amount: float, payment_method: str, session: Session
) -> dict[str, Any]:
...
def _notify_admin_audit_body(self, audit_log_id: int, session: Session) -> dict[str, Any]:
...
def send_refund_sms(
    _self, refund_id: int, phone: str, message: str, _session=None
) -> dict[str, Any]:
...
def process_refund_payment(
    self, refund_id: int, amount: float, payment_method: str, session=None
) -> dict[str, Any]:
    if session is None:
        with sync_session_maker() as session:
            return _process_refund_payment_body(self, refund_id, amount, payment_method, session)
    return _process_refund_payment_body(self, refund_id, amount, payment_method, session)


@celery_app.task(bind=True, name="refund.notify_admin", max_retries=2)
def notify_admin_audit(self, audit_log_id: int, session=None) -> dict[str, Any]:
    if session is None:
        with sync_session_maker() as session:
            return _notify_admin_audit_body(self, audit_log_id, session)
    return _notify_admin_audit_body(self, audit_log_id, session)
