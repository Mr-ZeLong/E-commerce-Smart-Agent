...
| 业务组件 | `components/` | `DecisionPanel.tsx`、`TaskList.tsx`、`ConversationLogs.tsx`、`AgentConfigEditor.tsx`、`KnowledgeBaseManager.tsx`、`ComplaintQueue.tsx`、`ExperimentManager.tsx`、`AnalyticsV2.tsx` 等 |
| 通用 UI | `../../components/ui/` | shadcn/ui 组件（Button、Card、Input、Sheet 等） |
| API 封装 | `../../lib/api.ts` | 统一 axios 实例与拦截器 |
| 服务端状态 | `../../hooks/` | `useAgentConfig.ts`、`useKnowledgeBase.ts`、`useTasks.ts`、`useNotifications.ts`、`useComplaints.ts`、`useExperiments.ts`、`useAnalytics.ts` 等 TanStack Query 封装 |
