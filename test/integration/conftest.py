...
def test_order(test_user: User) -> Order:
    """创建测试订单（不使用数据库）"""
    assert test_user.id is not None, "test_user fixture must have id set"
    order = Order(
        id=1,
        order_sn="SN20240001",
...
def test_refund_application(
    ...
    """创建测试退款申请（不使用数据库）"""
    assert test_user.id is not None, "test_user fixture must have id set"
    assert test_delivered_order.id is not None, "test_delivered_order fixture must have id set"
...
def admin_auth_token(test_admin: User) -> str:
    """生成管理员的认证 Token"""
    assert test_admin.id is not None, "test_admin fixture must have id set"
